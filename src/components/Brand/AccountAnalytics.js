...
const Contacts = () => {
  ...
  const fetchData = async (page = 1) => {
    setLoading(true);
    try {
      const res = await axios.post(baseUrl + "/get-all-campaigns", {
  ...
  }, [user.user_id, currentPage]); // Refetch data when user, page, or filter changes
...
{selectedContacts.length > 0 && (
  <Box display="flex" justifyContent="flex-start" ml={2}>
    ...
    >
          ...
          <TableBody>
            {campaignData.map((campaign, index) => (
              <TableRow key={campaign.id}>
                <TableCell padding="checkbox">
                  <Checkbox
                    checked={selectedContacts.includes(campaign.campaignId)}
                ...
                <TableCell>{campaign.emailsDelivered}</TableCell>
                <TableCell>{campaign.emailsOpened}</TableCell>
                <TableCell>{campaign.emailsBounced}</TableCell>
      ...
      <Typography mt={2}>Total Contacts: {totalCampaigns}</Typography>
      <Pagination
